...
const carrito = (id) =>{
    if(carro.length == 0){$("#titulo-cart").removeClass("oculto")}
    ...
    // console.log(carro[length - 1])
    $(".items-cart").prepend(
        `
        <div class='item-cart'>
        <div class='description-item'>
          <img class='img-cart' src='${carro[carro.length - 1 ].imagen}'/>
          <p>${carro[carro.length - 1 ].name}</p>
        </div>
        <input type="number" value="1" min="1">
        <p>Precio ${carro[carro.length - 1 ].precio}</p>
