...
const carrito = (id) =>{
    ...
    let cantidadItemSeleccionado = cantidad.value
    $(".finalizarCarrito").show()
    $(".finalizarCarrito").show()
    $(".total").show() 
    $("#titulo-cart").show()
    if(carroComprador.length == 0){$("#titulo-cart").removeClass("oculto")}
    ...
    for (let i = 0; i <productos.length; i++){
        if(id == productos[i].id){
            ...
            if(!estaEnElCarrito(id)){
                ...
                $(".items-cart").prepend(
                    `
                    <div class='item-cart' id='${carroComprador[carroComprador.length - 1 ].name}'>
                    <div class='description-item'>
                      <img class='img-cart' src='${carroComprador[carroComprador.length - 1 ].imagen}'/>
                      <h3>${(carroComprador[carroComprador.length - 1 ].name).toUpperCase()}</h3>
                    </div>
                    <h5>Cantidad: ${carroComprador[carroComprador.length - 1].cantidad}</h5>
                    <h5>SUbtotal: $${carroComprador[carroComprador.length - 1 ].precio * carroComprador[carroComprador.length - 1].cantidad}</h5>
                    <button id='${carroComprador[carroComprador.length - 1 ].id}' class='BorrarItemCart' onclick=borrarItemCarrito(this.id)>BORRAR ITEM</button>
                </div>`
                )
                actualizarTotal()
                actualizarCantidadNavBar("modificar")
            }
        }
...
const estaEnElCarrito = (idItem) => {
    for(let i=0; i<carroComprador.length; i++){
        if(carroComprador[i].id == idItem){
            swal("Ya fue añadido al carrito previamente","", "warning");
            return true
        }
    }
}

const borrarItemCarrito = (itemBorrado) =>{
    
    let encontrado = carroComprador.filter( (itemCarro) => {
        return itemCarro.id == itemBorrado
    })
    ...
    if(carroComprador.length == 0){ocultarElementosCarrito()}
    actualizarTotal()
...
const actualizarTotal = () => {
    let total = 0
    carroComprador.map(valor =>{
        total += valor.cantidad * valor.precio
    })
    $(".total").html('TOTAL CARRITO: $' +total)
}
...
const actualizarCantidadNavBar = () => {
